...
export async function POST(req: NextRequest) {
  // Client dibuat di DALAM fungsi agar tidak error saat build
  ...
  try {
    ...
    if (pertanyaanRows.length === 0) {
      return NextResponse.json(
        { error: 'Tidak ada data soal yang ditemukan. Pastikan kolom No diisi dengan angka.' },
        { status: 400 }
      )
    }
    ...
    if (pertanyaan.length === 0) {
      return NextResponse.json(
        { error: 'Tidak ada soal valid. Pastikan kolom Pertanyaan, Pilihan A, dan Pilihan B terisi.' },
        { status: 400 }
      )
    }
