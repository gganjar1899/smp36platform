...
export default function NilaiLegerAdminPage() {
  ...
  // Guru pengampu kelas+mapel yang sedang dipilih (untuk tanda tangan leger)
  useEffect(() => {
  ...
  const belumDinilai = siswaList.filter(s => getNilaiAkhir(s.id) === null).length
  ...
  return (
    <div className="p-3 sm:p-4 lg:p-6">
      ...
      {/* Tabel Leger (read-only, admin memantau — input tetap lewat guru) */}
                    ...
                    return (
                      <tr key={s.id} className="hover:bg-blue-50/20 transition">
                        ...
                        {Array.from({ length: jmlFormatif }, (_, j) => (
                          <td key={j} className="px-2 py-2 text-center text-xs text-gray-600">{nilaiMap[s.id]?.[`F${j + 1}`] ?? '-'}</td>
                        ))}
                        <td className="px-2 py-2 text-center font-semibold text-blue-700 text-xs bg-blue-50">{rtf ?? '-'}</td>
                        <td className="px-2 py-2 text-center text-xs text-gray-600">{nilaiMap[s.id]?.S1 ?? '-'}</td>
