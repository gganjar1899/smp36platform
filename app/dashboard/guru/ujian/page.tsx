...
const emptyForm = {
  judul: '', mapelId: '', kelasIds: [] as string[], jenisUjian: 'ulangan_harian',
...
export default function KelolaUjianGuruPage() {
  ...
  const handleSimpan = async () => {
    ...
    // Satu ujian dibikin per kelas yang dicentang, masing-masing dapet token unik sendiri
    // (soal ujian sama, tapi token beda per kelas — biar guru bisa umumin token per kelas terpisah)
    let gagal = 0
    for (const kelasId of form.kelasIds) {
      const { data: ujianBaru, error } = await supabase.from('ujian').insert({
        judul: form.judul, mapel_id: form.mapelId, kelas_id: kelasId, dibuat_oleh: guruId,
    ...
    if (gagal > 0) alert(`${gagal} dari ${form.kelasIds.length} kelas gagal dibuatkan ujian. Cek koneksi lalu coba lagi buat kelas yang gagal.`)
        ...
        <div className="bg-white rounded-xl border border-blue-100 shadow-sm p-4 md:p-6 mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            ...
            <div className="col-span-2">
                  ...
                  return (
                    <button key={k.id} type="button"
                      ...
                      className={`px-2.5 py-1 rounded-lg text-xs font-medium transition ${
                        dipilih ? 'bg-[#1a3a6b] text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                      }`}>
                      {k.nama}
                    </button>
                  )
              ...
              <p className="text-[11px] text-gray-400 mt-1">Centang beberapa kelas buat bikin ujian yang sama sekaligus — tiap kelas dapet token sendiri-sendiri.</p>
            </div>
            <div>
          ...
          <div className="flex gap-3 justify-end">
            ...
            <button onClick={handleSimpan} disabled={saving}
              ...
              {saving ? 'Menyimpan...' : form.kelasIds.length > 1 ? `Buat Ujian untuk ${form.kelasIds.length} Kelas (jadi Draft)` : 'Buat Ujian (jadi Draft)'}
            </button>
          </div>
        </div>
      )}
