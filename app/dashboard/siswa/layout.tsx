...
const menuGroups = [
  {
    ...
    items: [
      ...
      { href: '/dashboard/siswa/materi', label: 'Materi', exact: false, icon: (
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/dashboard/siswa/hasil-ujian', label: 'Hasil Ujian', exact: false, icon: (
    ...
    async function loadUser() {
      try {
        ...
        if (data.loggedIn) {
          ...
          if (data.role === 'siswa' && data.siswa?.kelasNama) {
          ...
          {!collapsed && (
            <div className="flex items-center gap-3 px-3 py-2 rounded-lg bg-gray-50 mb-2">
              ...
              <div className="overflow-hidden flex-1">
                ...
                <p className="text-[10px] text-gray-400">{kelasNama ? `Kelas ${kelasNama}` : 'Siswa SMPN 36'}</p>
              </div>
            </div>
          )}
